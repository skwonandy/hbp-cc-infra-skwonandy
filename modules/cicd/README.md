...
**ECR**: API / worker / frontend 用リポジトリを環境ごとに作成する（`modules/cicd` の main.tf で定義）。`create_worker_repository` が `false` のときは worker 用 ECR を作成しない（min 環境で使用）。push 時イメージスキャン有効。
