...
**ECR**: API / worker 用リポジトリを環境ごとに作成する（`modules/cicd` の main.tf で定義）。フロントは S3 + CloudFront で配信するため ECR は作成しない。`create_worker_repository` が `false` のときは worker 用 ECR も作成しない（dev 環境等で使用）。push 時イメージスキャン有効。
